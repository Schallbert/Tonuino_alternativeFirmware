...
protected:
    ...
    virtual void SetUp()
    {
        m_pNfcMock = new NiceMock<Mock_Nfc>();
        m_pNfcControl = new NfcControl(*m_pNfcMock, m_messageHandlerMock);
...
protected:
    NiceMock<Mock_Nfc> *m_pNfcMock{nullptr};
...
{
    ON_CALL(*m_pNfcMock, writeTag(_, _)).WillByDefault(Return(true));
...
{
    Folder resultFolder;
    ON_CALL(*m_pNfcMock, readTag(_, _)).WillByDefault(Return(false));
...
{
    Folder resultFolder;
    ON_CALL(*m_pNfcMock, readTag(_, _)).WillByDefault(Return(true));
    EXPECT_FALSE(m_pNfcControl->readFolderFromTag(resultFolder));
}
...
{
    Folder resultFolder;
    ON_CALL(*m_pNfcMock, readTag(4, _)).WillByDefault(Return(true));
...
{
    Folder resultFolder;
    EXPECT_CALL(*m_pNfcMock, readTag(_, arrayByteCompare(
                                            fakeBufferData,
                                            NFCTAG_MEMORY_TO_OCCUPY)));
...
{
    Folder resultFolder;
    m_pNfcMock->DelegateToFake(); // Delegates readTag() call to fake object
...
TEST_F(NfcCtrlTagPresence, noTag_returnsNoTag)
{
    ...
    ON_CALL(*m_pNfcMock, getTagPresence()).WillByDefault(Return(tagPresence));
...
{
    NfcControl myTest{*m_pNfcMock, m_messageHandlerMock};
    NfcControl_interface::eTagState tagPresence = NfcControl_interface::NEW_UNKNOWN_TAG;
    ON_CALL(*m_pNfcMock, getTagPresence()).WillByDefault(Return(tagPresence));
    m_pNfcMock->DelegateToFake(); // will return known card cookie

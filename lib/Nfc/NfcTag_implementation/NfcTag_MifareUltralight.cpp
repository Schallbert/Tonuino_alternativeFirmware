...
bool NfcTag_MifareUltralight::writeTag(byte blockAddress, byte *dataToWrite)
{
    ...
    for (uint8_t i = 0; i < (NFCTAG_MEMORY_TO_OCCUPY/MIFARE_UL_BLOCK_SIZE); ++i)
    {
        ...
        NfcTag_interface::copyArray(buffer, dataToWrite + (i * MIFARE_UL_BLOCK_SIZE), NFCTAG_MEMORY_TO_OCCUPY);
        // only the first 4 bytes are actually written. Rest is kept 0.
        status &= m_pMfrc522->tagWrite(blockAddress + (i * MIFARE_UL_BLOCK_SIZE), buffer, NFCTAG_MEMORY_TO_OCCUPY);
    }
    m_pMfrc522->tagHalt();
    return status;
}
